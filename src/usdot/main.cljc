(ns usdot.main
  "Kotodama WASM entrypoint for the Usdot clean-room actor (L4) — Clojure port.
  ...
  facts (`usdot.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Declaration" :plural "declarations" :id-prefix "usdot_dec"
    :fields [:declarantId :type :hsCode :status]     :required [:type :hsCode]
    :coerce {}                       :refs {}}
   {:entity "Shipment"    :plural "shipments"    :id-prefix "usdot_shi"
    :fields [:declarationId :carrier :origin :destination] :required [:carrier :origin]
    :coerce {}                       :refs {:declarationId "Declaration"}}
   {:entity "Manifest"    :plural "manifests"    :id-prefix "usdot_man"
    :fields [:shipmentId :vessel :containerCount]    :required [:vessel :containerCount]
    :coerce {:containerCount :int}   :refs {:shipmentId "Shipment"}}
   {:entity "Tariff"      :plural "tariffs"      :id-prefix "usdot_tar"
    :fields [:hsCode :rate :jurisdiction]            :required [:hsCode :rate]
    :coerce {:rate :float}           :refs {}}
   {:entity "Permit"      :plural "permits"      :id-prefix "usdot_per"
    :fields [:declarationId :type :status]           :required [:type :status]
    :coerce {}                       :refs {:declarationId "Declaration"}}
   {:entity "Party"       :plural "parties"      :id-prefix "usdot_par"
    :fields [:name :role :taxId]                     :required [:name :role]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"usdot.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "usdot-compat" :tier tier :entities entities} 200])
